...
## Deliver commit boundaries

Within `deliver`, commit at meaningful phase boundaries when files changed.

Minimum expectation:

- commit after roadmap creation or roadmap revision
- commit after accepted implementation batches
- commit after explicit refactor passes
- commit after finalize artifacts are written

Use focused commits rather than one giant end-of-run commit whenever the workflow naturally pauses for review.

If a deliver checkpoint is awaiting user review, the commit should already exist before you ask the user to review it.
